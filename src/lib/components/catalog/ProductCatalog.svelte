<script lang="ts">
  ...
  const filtered = $derived.by(() => {
    return products.filter((product: ProductTableRow) => {
...
<section id="katalog" class="section-shell py-8 md:py-10">
  ...
  <p class="mt-2 text-sm text-[#624c3b]">Filter berdasarkan jenis cap dan cari berdasarkan warna.</p>
  ...
  {:else}
    <div class="mt-5 grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
      {#each filtered as product}
        <ProductCard {product} onAsk={onAskProduct} />
      {/each}
    </div>
  {/if}
</section>
